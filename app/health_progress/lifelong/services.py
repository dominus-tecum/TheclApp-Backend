...
class LifelongService:
    ...
    def create_or_update_entry(self, entry_data: Dict[str, Any]) -> LifelongEntry:
    ...
    def get_all_entries(self) -> List[LifelongEntry]:
        return self.db.query(LifelongEntry).order_by(
            LifelongEntry.submission_date.desc()
        ).all()

    def get_patient_entries(self, patient_id: int) -> List[LifelongEntry]:
    ...
    def get_entry_by_date(self, patient_id: int, date: str):
        return self.db.query(LifelongEntry).filter(
            LifelongEntry.patient_id == patient_id,
    ...
    def check_existing_entry(self, patient_id: int, date: str) -> bool:
        return self.get_entry_by_date(patient_id, date) is not None
    
    def delete_entry(self, entry_id: int) -> bool:
        entry = self.db.query(LifelongEntry).filter(LifelongEntry.id == entry_id).first()
        if entry:
